    ...
    bool isValidSudoku(vector<vector<char>>& board) {
        ...
        for(int i=0;i<9;i++){
            for(int j=0;j<9;j++){
                ...
                int curr = board[i][j]; 
                //we can get the value at given index to check that if it is already 
                ...
                //for bloack no use  = (i/3)*3 + (nj/3)

                if( rows[i].count(curr) || cols[j].count(curr) ||
                    blocks[(i/3)*3 + j/3].count(curr)
                    ){
                    return false; //we return ans as false because the value currently we have is already present in set
                }
